    <script>
      ...
      // Simpan nama kelas saat ini di sessionStorage untuk navigasi kembali
      ...
      santriGallery.innerHTML = '<p class="text-center text-gray-500 py-10">Tidak ada santri di kelas ini.</p>';
          ...
          // Perbaikan ada di sini: Gunakan URL placeholder jika santri.foto tidak ada
      ...
      if (hafalan.length > 0) {
          hafalan.forEach(h => {
              ...
              row.innerHTML = `
                  <td class="py-4 px-6 text-sm text-gray-200">${new Date(h.tanggal).toLocaleDateString('id-ID')}</td>
                  <td class="py-4 px-6 text-sm text-gray-200 font-medium">${h.setoran}</td>
                  <td class="py-4 px-6 text-sm text-gray-400">${h.jenis}</td>
                  <td class="py-4 px-6 text-sm">
                      <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${h.penilaian === 'Lancar' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}">
                          ${h.penilaian}
                      </span>
                  </td>
              `;
      ...
      async function renderGuruDashboard() {
          ...
          if (listContainer) {
              ...
              pondokData.santri.sort((a, b) => a.nama.localeCompare(b.nama)).forEach(santri => {
                  ...
                  item.innerHTML = `
                  <img src="${santri.foto}" alt="${santri.nama}" class="w-16 h-16 rounded-full object-cover mb-2" onerror="this.onerror=null;this.src='https://placehold.co/400x400?text=Error';">
                  <p class="font-bold text-gray-100">${santri.nama}</p>
                  <p class="text-sm text-gray-400 mb-2">Kelas ${santri.kelas}</p>
                  <div class="flex space-x-2">
                      <button onclick="showGuruForm(${santri.id})" class="px-2 py-1 text-xs font-semibold text-black bg-yellow-400 rounded-md hover:bg-yellow-500">Ubah Data</button>
                      <button onclick="deleteSantri(${santri.id})" class="px-2 py-1 text-xs font-semibold text-white bg-red-600 rounded-md hover:bg-red-700">Hapus</button>
                  </div>
                  `;
      ...
      async function deleteSantri(santriId) {
          ...
          if (result) {
              const saveSuccess = await saveData('/api/delete-santri', {
                  santriId
              });
              if (saveSuccess) {
      ...
      async function renderGuruList() {
          ...
          pondokData.guru.sort((a, b) => a.nama.localeCompare(b.nama)).forEach(guru => {
              ...
              row.innerHTML = `
              <td class="py-4 px-6 text-sm font-medium text-gray-100">${guru.nama}</td>
              <td class="py-4 px-6 text-sm text-gray-400">${guru.mengajar_kelas}</td>
              <td class="py-4 px-6 text-sm font-medium space-x-2">
                  <button onclick="editGuru(${guru.id})" class="text-yellow-400 hover:text-yellow-500">Ubah</button>
                  <button onclick="deleteGuru(${guru.id})" class="text-red-600 hover:text-red-700">Hapus</button>
              </td>
              `;
          });
      }
      ...
      document.getElementById('guru-form').addEventListener('submit', async function(e) {
          ...
          const saveSuccess = await saveData('/api/save-guru', {
              pondokId,
              guruData
          });
          if (saveSuccess) {
      ...
      async function deleteGuru(guruId) {
          ...
          if (!result) return;
          const pondokId = sessionStorage.getItem('currentPondokId');
          const saveSuccess = await saveData('/api/delete-guru', {
              pondokId,
              guruId
          });
          if (saveSuccess) {
      ...
      document.getElementById('hafalan-form').addEventListener('submit', async function(e) {
          ...
          const saveSuccess = await saveData('/api/save-hafalan', {
              pondokId,
              hafalanData: newHafalan
          });
          if (saveSuccess) {
      ...
      async function deleteHafalan(hafalanId, santriId) {
          ...
          if (!result) return;
          const pondokId = sessionStorage.getItem('currentPondokId');
          const saveSuccess = await saveData('/api/delete-hafalan', {
              pondokId,
              hafalanId
          });
          if (saveSuccess) {
      ...
      // --- FEED LOGIC ---
      function showFeedPage(source) {
          document.getElementById('feed-container').innerHTML = '';
          document.getElementById('post-form-modal').classList.add('hidden');
          const postButton = document.getElementById('feed-page').querySelector('header button.bg-yellow-400');
          
          if (source === 'public' || sessionStorage.getItem('role') === 'wali') {
      ...
      async function renderFeed() {
          ...
          const allPondokData = appDataCache; // Assuming a global cache for all pondok data
              ...
              postEl.innerHTML = `
                  <div class="flex items-center space-x-4 mb-4">
                      ...
                      <div>
                          ...
                          <p class="text-xs text-gray-400">${new Date(post.timestamp).toLocaleString('id-ID')}</p>
                      </div>
                      ${sessionStorage.getItem('role') === 'guru' ? `<button onclick="deletePost(${post.id})" class="ml-auto text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>` : ''}
                  </div>
                  <p class="text-gray-300 mb-4">${post.text}</p>
                  ${mediaHtml}
                  <div class="flex items-center space-x-4 mt-4 text-gray-400">
                      <button onclick="likePost(${post.id})" class="flex items-center space-x-1 hover:text-yellow-400">
                          <i class="fa-solid fa-heart"></i>
                          <span>${post.likes}</span>
                      </button>
                      <button onclick="sharePost(${post.id})" class="flex items-center space-x-1 hover:text-yellow-400">
                           <i class="fa-solid fa-share"></i>
                      </button>
                      <div class="flex-grow"></div>
                      <button onclick="toggleComments(${post.id})" class="flex items-center space-x-1 hover:text-yellow-400">
                          <i class="fa-solid fa-comment"></i>
                          <span>${post.comments ? post.comments.length : 0}</span>
                      </button>
                  </div>
                  <div id="comments-container-${post.id}" class="comments-container hidden mt-4 border-t border-gray-700 pt-4">
                      <div id="comment-list-${post.id}" class="space-y-2">
                      </div>
                      <form onsubmit="addComment(event, ${post.id})" class="mt-4 flex space-x-2">
                          <input type="text" id="comment-input-${post.id}" placeholder="Tulis komentar..." class="flex-grow p-2 border rounded-md bg-gray-700 text-gray-200">
                          <button type="submit" class="bg-yellow-400 text-black px-4 py-2 rounded-md hover:bg-yellow-500">Kirim</button>
                      </form>
                  </div>
              `;
      ...
      function sharePost(postId) {
          ...
          const shareUrl = `https://santriq.app/post/${postId}`; // Example share URL
          
          if (navigator.share) {
              navigator.share({
                  title: 'Postingan SantriQ',
                  text: shareText,
                  url: shareUrl
      ...
      function playWelcomeAnimation() {
          ...
          setTimeout(() => {
              subtitle.classList.remove('translate-y-10', 'opacity-0');
          ...
          try {
              ...
              for (const [nama, waktu] of Object.entries(jadwalPenting)) {
                  ...
                  <div class="bg-gray-700 p-4 rounded-lg shadow-sm text-center">
                      <p class="font-semibold text-yellow-400">${nama}</p>
                      <p class="text-2xl font-bold text-white">${waktu.substring(0, 5)}</p>
                  </div>`;
              }
              setupPrayerAlarm();
          } catch (error) {
